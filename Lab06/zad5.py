...
class Measurements:
    def __init__(self, dir_path):
        ...
        self._files_ = self.identify_files()
        self._loaded_files = {}
    ...
    def lazy_load_file(self, filename: str):
        ts_list = []
        ...
        dates_list = df[df.columns[0]].iloc[5:].astype(str).tolist()

        for station_column in df.columns[1:]:
            values_list = df[station_column].iloc[5:].astype(float).tolist()
    ...
    def get_by_station(self, station_code: str):
        ...
        other_files = [f for f in self._files_ if f["idx_name"] not in self._loaded_files]

        for f in other_files:
            f_res = []
            file_series = self.lazy_load_file(f["filename"])
            idx_name = f["idx_name"]
            ...
            for ts in file_series:
                if ts.station_code == station_code:
                    f_res.append(ts)
                    result.append(ts)
            if idx_name in self._loaded_files:
                self._loaded_files[idx_name].extend(f_res)
            else:
                self._loaded_files[idx_name] = f_res
    ...
    def detect_all_anomalies(self, validators: list[SeriesValidator], preload: bool = False):
        ...
        for f in self._loaded_files:
            for ts in self._loaded_files[f]:
                series_anomalies = []
